...
public class UtilsCodeGenerator {
	...
	public static void setLinkMethods(CompilationUnit c, State s) {
		...
		for (Edge edge : s.getLinks()) {
			...
			String towards = UtilsStaticAnalyzer.getStateNameFromId(
					edge.getTo());
	 ...
	 * @param s
	 */
	public static void setGettersMethodsForAssertions(CompilationUnit c, State s) {

		if (s.getDiffs() == null) {
			return;
		}

		for (abstractdt.Diff d : s.getDiffs()) {
